...
function contruirCardNoticia(titulo, descrição, url, imagem, data) {
...
async function noticiasRecentes(quantidade = 4) {
    try {
        ...
        noticiasFiltradas.forEach((element) => containerNoticias.appendChild(contruirCardNoticia(element.titulo, element.descricao, element.url, element.imagem, element.data)));
    }
    catch (error) {
